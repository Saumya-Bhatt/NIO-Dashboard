...
if st.sidebar.checkbox('Open Console',False,key=1):
    ...
    if st.checkbox('Open Offline Map',key='offline_map'):
        ...
        if st.checkbox('Read Instructions',False,key='inst'):
            ...
            st.markdown('__2.__ Enter the file which contains the coordinates of the __top left__ and __bottom right__ points of the image to be used.')
        ...
        if UPLOADED_FILE is not None:
            ...
            st.dataframe(data_frame(loc_data))


    
    if st.checkbox('Open online Map',key='online_map'):
        st.markdown('Upload a locations file. The dashboard will display the map of the last location file entered unless a new one is uploaded.')

        ONLINE_FILE = st.file_uploader('Choose a locations file [.TXT]',type='TXT',key='online_file')
        if ONLINE_FILE is not None:
            ONLINE_LOC = cleanFile(ONLINE_FILE)
            
            with open('location.json','r') as jsonFile:
                dump = json.load(jsonFile)

            dump['home']['latitude'] = [float(ONLINE_LOC[0][0])]
            dump['home']['longitude'] = [float(ONLINE_LOC[0][1])]
            dump['boat']['latitude'] = [float(ONLINE_LOC[1][0])]
            dump['boat']['longitude'] = [float(ONLINE_LOC[1][1])]
            dump['c-bot']['latitude'] = [float(ONLINE_LOC[2][0])]
            dump['c-bot']['longitude'] = [float(ONLINE_LOC[2][1])]
            ...
            st.dataframe(data_frame(ONLINE_LOC))
